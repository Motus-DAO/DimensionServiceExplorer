...
export type DMClient = {
    ...
    SendSilent: (pubkey: Uint8Array, dmToken: number, cmixParams: Uint8Array) => Promise<Uint8Array>;
    SendInvite: (pubkey: Uint8Array, dmToken: number, inviteToChannelJson: Uint8Array, message: string, host: string, cmixParamsJSON: Uint8Array) => Promise<Uint8Array>;
    DeleteMessage: (partnerPubKeyBytes: Uint8Array, partnerToken: number, targetMessageIdBytes: Uint8Array, cmixParamsJSON: Uint8Array) => Promise<Uint8Array>;
    Send: (partnerPubKeyBytes: Uint8Array, partnerToken: number, messageType: number, plaintext: Uint8Array, leaseTimeMS: number, cmixParamsJSON: Uint8Array) => Promise<Uint8Array>;
};
...
export type XXDKUtils = {
    ...
    NewSynchronizedCmix: (ndf: string, storageDir: string, remoteStoragePrefixPath: string, password: Uint8Array, remoteStore: RemoteStore) => Promise<void>;
    LoadCmix: (storageDirectory: string, password: Uint8Array, cmixParams: Uint8Array) => Promise<CMix>;
    LoadSynchronizedCmix: (storageDirectory: string, password: Uint8Array, remoteStore: RemoteStore, cmixParams: Uint8Array) => Promise<CMix>;
    ...
    NewChannelsManagerWithIndexedDb: (cmixId: number, wasmJsPath: string, privateIdentity: Uint8Array, extensionBuilderIDsJSON: Uint8Array, notificationsId: number, callbacks: ChannelManagerCallbacks, channelDbCipher: number) => Promise<ChannelManager>;
    NewDMClientWithIndexedDb: (cmixId: number, notificationsId: number, cipherId: number, wasmJsPath: string, privateIdentity: Uint8Array, eventCallback: DMClientEventCallback) => Promise<DMClient>;
    NewDatabaseCipher: (cmixId: number, storagePassword: Uint8Array, payloadMaximumSize: number) => RawCipher;
    LoadChannelsManagerWithIndexedDb: (cmixId: number, wasmJsPath: string, storageTag: string, extensionBuilderIDsJSON: Uint8Array, notificationsId: number, callbacks: ChannelManagerCallbacks, channelDbCipher: number) => Promise<ChannelManager>;
